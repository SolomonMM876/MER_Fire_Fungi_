...
# Plot
ggplot(Beta_PA_phy_soil, aes(x = family, y = Beta, fill=family)) +
  ...
  labs(y = "Beta estimates", x = "family", title = "Beta values by family for PA model") +
  theme_minimal() 


#PA model
model <- lm(Beta ~ 0+  family, data = Beta_PA_phy_soil)
...
# Tukey pairwise contrasts: Type within each family
contrasts_type_within_family <- emmeans(model, pairwise ~ family, adjust = "sidak")

# Extract significant results
sig_labels <- contrasts_type_within_family$contrasts %>%
  as.data.frame() %>%
  filter(p.value < 0.1) %>%
  mutate(
    p.label = case_when(
 ...
 family_max <- Beta_PA_phy_soil %>%
  group_by(family) %>%
  summarise(y.position = max(Beta, na.rm = TRUE)+0.05)

sig_results <- sig_labels %>%
  left_join(family_max, by = c("xmin"="family") )



# Main boxplot
Beta_PA_phy_soil %>% 
  ggplot( aes(x = family, y =Beta, fill=family)) +
  geom_boxplot(alpha = 0.7, outlier.shape = NA, width=.4, # wider dodge increases sPAcing
  ) +
  geom_jitter(alpha = 0.9, size = 2, width = 0.05 # must match boxplot dodge
  ) +
  labs(y = "Beta coefficent", x = "family", title = "Beta values by family PA Model all taxa") +
  theme_minimal() +
...
# Additive model
mod_add <- lm(abs_Beta ~ log_ratio + family , data = RA_ratio_PA)

# Interaction model
mod_int <- lm(abs_Beta ~ log_ratio * family , data = RA_ratio_PA)

anova(mod_add, mod_int)  # Likelihood Ratio Test
AIC(mod_add, mod_int)    # Model comparison

model <- lm(Beta ~ log_ratio * family , data = RA_ratio_PA)
...
# Tukey pairwise contrasts: Type within each family
contrasts_type_within_family <- emmeans(model, pairwise ~ family*log_ratio, adjust = "sidak")
...
family_max <- RA_ratio_PA %>%
  group_by(family) %>%
  summarise(y.position = max(Beta, na.rm = TRUE)+0.05)

sig_results <- sig_labels %>%
  left_join(family_max, by = c("xmax"="family") )


RA_ratio_PA %>% 
filter(family %in% c("Tricholomataceae", "Claroideoglomeraceae", "Diversisporaceae")) %>% 
ggplot(aes(x = log_ratio, y = Beta, color = family)) +
  geom_point(size = 3, alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE, fullrange = TRUE) +
  labs(
    title = "Interaction model: Beta ~ log_ratio * family",
    y = "Beta fire effect",
...
# Plot
ggplot(Beta_COP_phy_soil, aes(x = family, y = Beta, fill=family)) +
  geom_boxplot(alpha = 0.7, outlier.shape = NA, width=.3,     position = position_dodge(width = 0.6)  # wider dodge increases spacing
  ) +
  geom_jitter( alpha = 0.9, size = 1,     position = position_dodge(width = 0.6)  # must match boxplot dodge
  ) +
  labs(y = "Beta estimates", x = "family", title = "Beta values by family for COP model") +
  theme_minimal() +
  scale_fill_brewer(palette = "Set2")


#COP model
model <- lm(Beta ~  family, data = Beta_COP_phy_soil)
