...
# Plot for pH#######
ggplot(Beta_PA_Site_soil, aes(x = Soil_pH, y = abs_Beta)) +
  geom_point( alpha = 0.9, size = 1 ) +
  geom_smooth()+
  labs(y = "Beta values per OTU", x = "Soil_pH", title = "Beta values by Soil_pH") +
  theme_minimal() + 
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1))

hist(Beta_PA_Site_soil$Soil_pH)
#PA model all
model <- lmer(Beta ~ Soil_pH+ (1|Site/Plot), data = Beta_PA_Site_soil)
...
hist(log10(Beta_PA_Site_soil$PO4))

# Plot for PO4
Beta_PA_Site_soil %>% 
  mutate(log_PO4=log10(PO4)) %>% 
ggplot( aes(x = log_PO4, y = abs_Beta)) +
  geom_point( alpha = 0.9, size = 1 ) +
  geom_smooth()+
  labs(y = "Beta values per OTU", x = "log_PO4", title = "Beta values by PO4") +
  theme_minimal() + 
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1))
...
#PA model all
model <- lmer(Beta ~ log_PO4+ (1|Site/Plot), data = Beta_PA_Site_soil %>%  mutate(log_PO4=log10(PO4)))
...
# Plot for NH4
ggplot(Beta_PA_Site_soil, aes(x = NH4, y = Beta)) +
  geom_point( alpha = 0.9, size = 1 ) +
  geom_smooth()+
  labs(y = "Beta values per OTU", x = "NH4", title = "Beta values by NH4") +
  theme_minimal() + 
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1))
...
#PA model all
model <- lmer(abs_Beta ~ NH4+ (1|Site/Plot), data = Beta_PA_Site_soil)
...
hist(log10(Beta_PA_Site_soil$NO3))

# Plot for NO3
Beta_PA_Site_soil %>% 
  mutate(log_NO3=log10(NO3)) %>% 
ggplot( aes(x = log_NO3, y = abs_Beta)) +
  geom_point( alpha = 0.9, size = 1 ) +
  geom_smooth()+
  labs(y = "Beta values per OTU", x = "log_NO3", title = "Beta values by NO3") +
  theme_minimal() + 
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1))
...
#PA model all
model <- lmer(abs_Beta ~ log_NO3+ (1|Site/Plot), data = Beta_PA_Site_soil%>% 
                mutate(log_NO3=log10(NO3)))
...
Anova_model<-Anova(model, test = "F")
Anova_model

Anova_P <- round(Anova_model$`Pr(>F)`, 3)
# Extract R2 values
r2_vals <- model_performance(model)
marg_r2 <- round(r2_vals$R2_marginal, 3)


# Add log_NO3 column and plot
Beta_PA_Site_soil %>%
  mutate(log_NO3 = log10(NO3)) %>%
  ggplot(aes(x = log_NO3, y = abs_Beta)) +
  geom_point(alpha = 0.9, size = 1) +
  ...
  labs(
    y = "Beta values per OTU",
    x = "log10(NO3)",
    title = "abs_Beta ~ log_NO3+ (1|Site/Plot)"
  ) +
  annotate("text", 
           x = min(log10(Beta_PA_Site_soil$NO3), na.rm = TRUE), 
           y = max(Beta_PA_Site_soil$abs_Beta, na.rm = TRUE), 
           hjust = 0,
           label = paste0("Marginal R² = ", marg_r2,
                          "\n Anova p= ", Anova_P),
           size = 4) +
  theme_minimal() +
  theme(
...
# Additive model
mod_add <- lmer(abs_Beta ~ log_ratio + log_NO3 + (1|Site/Plot) , data = RA_ratio_PA)

# Interaction model
mod_int <- lmer(abs_Beta ~ log_ratio * log_NO3 + (1|Site/Plot) , data = RA_ratio_PA)

anova(mod_add, mod_int)  # Likelihood Ratio Test
AIC(mod_add, mod_int)    # Model comparison

model <- lmer(abs_Beta ~ log_ratio * log_PO4 + (1|Site/Plot) , data = RA_ratio_PA)
...
library(ggeffects)
pred_PO4 <- ggpredict(model, terms = "log_PO4") %>% 
  mutate(PO4=10^x)
# Plot predictions with ggplot2
ggplot(pred_PO4, aes(x = PO4, y = predicted)) +
  geom_line(color = "blue", linewidth = 1) +
 # geom_point(data=RA_ratio_PA, aes(x=log_PO4, y=abs_Beta),alpha=.6)
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = 0.2, fill = "blue") +
  labs(
    title = "Effect of PO4 on abs_Beta",
    x = "PO4 (unit)",  # Adjust label as appropriate
...
Site_max <- RA_ratio_PA %>%
  group_by(Site) %>%
  summarise(y.position = max(Beta, na.rm = TRUE)+0.05)

sig_results <- sig_labels %>%
  left_join(Site_max, by = c("xmax"="Site") )




ggplot(RA_ratio_PA, aes(x = log_PO4, y = abs_Beta)) +
  geom_point(size = 3, alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE, fullrange = TRUE) +
  labs(
    title = "Interaction model: abs_Beta ~ log_ratio + log_PO4",
    y = "|Beta fire effect|",
    x = "log10(PO4)") +
