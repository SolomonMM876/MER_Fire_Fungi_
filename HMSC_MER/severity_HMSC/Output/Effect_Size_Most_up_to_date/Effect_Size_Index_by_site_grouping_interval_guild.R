...
# -----------------------------------------------
# Run across all variables for each guild
# -----------------------------------------------
anova_results_AM <- vars %>%
  map_dfr(run_anova, data = Beta_PA_Veg_soil_AM) %>%
  mutate(guild2 = "Arbuscular Mycorrhizal")    # <-- label added

anova_results_ECM <- vars %>%
  map_dfr(run_anova, data = Beta_PA_Veg_soil_ECM) %>%
  mutate(guild2 = "Ectomycorrhizal")            # <-- label added

# -----------------------------------------------
# Bind together
# -----------------------------------------------
anova_results_site_vars <- bind_rows(anova_results_AM, anova_results_ECM)
