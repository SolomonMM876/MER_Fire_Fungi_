...
MER_fire_regime<-read_rds("Processed_data/metadata/fire/Fire_regime_plot.Rdata") %>% 
  select(Site,Plot,fire_severity_num)
...
data <-All%>%
  mutate(Fire_Treatment=as.factor(Fire_Treatment),
...
#set factor order
data$Fire_Treatment <- factor(data$Fire_Treatment, levels = c("U", "B"))

#select factors
XData <-data %>% 
  select(Fire_Treatment,freq_AM, freq_ECM,total_reads)#
...
names(models) = c("Fire_Soil_MER_PA")
# export workspace
save(models,thin, samples, nChains, transient,nParallel, file = file.path(dataDir, "unfitted_models_fire.RData"))
